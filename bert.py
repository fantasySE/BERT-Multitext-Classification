    ...
    def _read_csv(cls, input_file, quotechar=None):
        ...
        with open(input_file, 'r') as f:
            ...
            for i in range(len(dicts['question'])):
                ...
                dicts['question'][i] = ' '.join(dicts['question'][i])
        return dicts
...
class MyPro(DataProcessor):
    ...
    def get_labels(self):
        #return [0, 1]
        return  ['Adviserefund', 'Alreadyrefunded', 'Buyerpay', 'Canbechanged', 'Cancellation', 'Cannotbechanged', 'DelayOTHERSITES', 'DelayUS',
                 'Deliverfailed', 'Faster1', 'Faster2', 'Howtorequest', 'Instock', 'IntransitGB', 'IntransitOTHERSITES', 'Invoiceleadtime',
                 'Missing', 'Notacceptedreturn', 'Notracking', 'Ourfault1', 'Ourfault2', 'Outstock', 'Photoproof', 'Plswait',
                 'Redelivery', 'Refund', 'Return_tracking', 'ShippingCharge1', 'ShippingCharge2', 'Tracking', 'Wishtochange', 'Wishtowait', 'mark_as_urgent',
                 'replacement', 'when_instock']
    ...
    def _create_examples(self, dicts, set_type):
        ...
        for i in range(len(dicts)):
            guid = "%s-%s" % (set_type, i)
            text_a = dicts.iloc[i]['question']
            label = dicts.iloc[i]['tag']
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
        return examples
...
def val(model, processor, args, label_list, tokenizer, device):
    ...
    precision = metrics.precision_score(gt, predict, average = 'micro' )
    # print(f1)
...
def test(model, processor, args, label_list, tokenizer, device):
    ...
    # print('预测的标签： \n', predict)
    precision = metrics.precision_score(gt, predict, average='micro')
    print('precision score in test set is {}'.format(precision))
...
#-------------------------导出结果------------------------
    ...
    with open(input_file, 'r') as f:
        reader = pd.read_csv(f)
        dicts = reader.ix[:, ['question', 'tag']]
    label_dict = {0: 'Adviserefund', 1: 'Alreadyrefunded', 2: 'Buyerpay', 3: 'Canbechanged', 4: 'Cancellation',
                  5: 'Cannotbechanged', 6: 'DelayOTHERSITES', 7: 'DelayUS',
                  8: 'Deliverfailed', 9: 'Faster1', 10: 'Faster2', 11: 'Howtorequest', 12: 'Instock', 13: 'IntransitGB',
                  14: 'IntransitOTHERSITES', 15: 'Invoiceleadtime',
                  16: 'Missing', 17: 'Notacceptedreturn', 18: 'Notracking', 19: 'Ourfault1', 20: 'Ourfault2',
                  21: 'Outstock', 22: 'Photoproof', 23: 'Plswait',
                  24: 'Redelivery', 25: 'Refund', 26: 'Return_tracking', 27: 'ShippingCharge1', 28: 'ShippingCharge2',
                  29: 'Tracking', 30: 'Wishtochange', 31: 'Wishtowait',
                  32: 'mark_as_urgent', 33: 'replacement', 34: 'when_instock'}

    g = []
    p = []
    for i in range(len(gt)):
        g.append(label_dict[gt[i]])
    for j in range(len(predict)):
        p.append(label_dict[predict[j]])

    result = pd.DataFrame({'question': dicts['question'], 'tag': g, 'predict': p},
                          columns=['question', 'tag', 'predict'])
...
def main():
    ...
    parser.add_argument("--data_dir",
                        default = '/home/fantasy/PycharmWorks/bert_pytorch/',
                        type = str,
                        #required = True,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            precision = val(model, processor, args, label_list, tokenizer, device)
            if precision > best_score:
                best_score = precision
                print('precison score in eval set is = {}'.format(precision))
                flags = 0
                checkpoint = {
                    'state_dict' : model.state_dict()
                }
                ...
                print('precison score in eval set is(no improvement) = {}'.format(precision))
                flags += 1
                if flags >= 6:
                    break
